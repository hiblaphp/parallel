...
describe('Parallel and Async Concurrency', function () {
    ...
    it('runs parallel and async tasks concurrently and completes under 2 seconds', function () {
        ...
        Promise::all([
            parallel(fn() => sleep(1))->then(function () use (&$output) { $output[] = 1; }),
            parallel(fn() => sleep(1))->then(function () use (&$output) { $output[] = 2; }),
            parallel(fn() => sleep(1))->then(function () use (&$output) { $output[] = 3; }),
            async(fn() => Hibla\sleep(0.5))->then(function () use (&$output) { $output[] = 4; }),
            async(fn() => Hibla\sleep(0.5))->then(function () use (&$output) { $output[] = 5; }),
            async(fn() => Hibla\sleep(0.5))->then(function () use (&$output) { $output[] = 6; }),
        ])->wait();
